...
def _ewma_cov(rets: np.ndarray, halflife: int = 30) -> np.ndarray:
    """EWMA covariance with exponential decay. Downweights old data."""
    T, N = rets.shape
...
class MyStrategy(StrategyBase):
    ...
    def fit(self, train_prices: np.ndarray, meta: PublicMeta, **kwargs) -> None:
        ...
        for hl in ensemble_halflives:
            ...
            # Build views (omega_scale=0.001 for tighter view confidence)
            views, omegas, q_list = [], [], []
            omega_scale = 0.001
            ...
            for s in [3, 4]:
                ...
                if q_val > 0:
                    ...
                    views.append(p)
                    omegas.append(np.var(rets @ p) * omega_scale)
                    q_list.append(q_val)
            ...
            if len(s3_others) > 0:
                ...
                if q_a23 > 0:
                    views.append(p_a23)
                    omegas.append(np.var(rets @ p_a23) * omega_scale)
                    q_list.append(q_a23)
            ...
            if len(s3_mid) > 0:
                ...
                if q_a09 > 0:
                    views.append(p_a09)
                    omegas.append(np.var(rets @ p_a09) * omega_scale)
                    q_list.append(q_a09)
        ...
        self._base_weights = _project_l1(w_ensemble)
